...
| `cymule-activation-http` | signal `WaitSourceDriver` | Axum, Tokio, SQLite | exact `cymule.activation-http-spool/1` physical generation with empty-only atomic initialization and no repair/import path, complete-row strict-canonical request-digest verification before selection/replay, durable ingress spool, fair parked-key cursor and indexed matching beyond arbitrary transport prefixes, injected authorization, response only after activation acknowledgement, exact reopen/redelivery, duplicate/conflict handling |
| `cymule-activation-timer` | timer `WaitSourceDriver` | SQLite through `rusqlite` | exact `cymule.activation-timer-store/2` physical generation with no `/1` reader/importer, complete schedule digest and strict-canonical row closure before selection/replay, durable schedules and selected targets, injected clock, selection-required acknowledgement, exact redelivery after restart until acknowledgement |
