...
| --- | --- | --- |
| [Legacy public release ref revocation](legacy-public-release-ref-revocation-v1.md) | Executed 2026-08-31; terminal readback complete | GitHub Releases and annotated tags `v0.1.1` through `v0.1.4`; these tag names are permanently retired |
...
| [Durable StateRoot generation 6](durable-state-root-generation-6.md) | Source implemented; operator execution pending; drain/export/recreate/requeue required | StateRoot/value `/4` stores are replaced by fresh `/6` authorities. Current Agent selector set: `cymule.agent/10`, `cymule.agent-command/4`, `cymule.agent-command-id/2`, `cymule.agent-command-receipt/6`, `cymule.agent-command-receipt-id/4`, `cymule.agent-target-claim-current/3`, `cymule.agent-target-claim-id/3`, `cymule.agent-target-claim-key/1`, `cymule.agent-target-claim-generation-record/1`, and `cymule.agent-target-claim-generation-key/1` |
