...
while [ "$iteration" -le "$CYMULE_SOAK_REPETITIONS" ]; do
  ...
  cargo test -p cymule-virtual --test public_durable \
  ...
  cargo test -p cymule-resource-object-store --lib \
    public_conformance_tests::object_store_chunk_retry_commit_and_read_are_exact -- --exact
