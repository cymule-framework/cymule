...
- Add the independent generation-bearing Agent target-claim family. Direct
  ...
  sidecars. This hard-cuts Agent to `/10`, command/receipt to `/4` and `/6`,
  command/receipt identity domains to `/2` and `/4`, target claim current/ID to
  `/3`, adds generation-record/key `/1`, keeps publication intent/reservation
  at `/2` and `/3`, and advances StateRoot/value to `/6`; retained `/4` stores
